...
export const calculateShowtimeStats = (showtimes: AdminShowtimeRow[]) => {
  ...
  const resolveShowDate = (item: AdminShowtimeRow) => {
    if (item.showDate) return dayjs(item.showDate);
  ...
  const today = showtimes.filter((item) => resolveShowDate(item).isSame(now, 'day')).length;
