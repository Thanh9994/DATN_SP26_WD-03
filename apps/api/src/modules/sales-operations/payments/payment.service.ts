...
export const paymentService = {
  async initPayment(
    ...
    console.log(`💰 Creating payment for booking ${bookingId}, amount: ${amount}`);
    ...
    const paymentUrl = await gateway.createUrl(payment._id.toString(), amount, ipAddr);
  ...
  async processIpn(method: string, data: any) {
    try {
      ...
      const amount = method === 'vnpay' ? Number(data.vnp_Amount) / 100 : Number(data.amount || 0);
