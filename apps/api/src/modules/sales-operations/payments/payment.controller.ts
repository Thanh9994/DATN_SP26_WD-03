...
export const createPaymentUrl = catchAsync(async (req: Request, res: Response) => {
  ...
  const paymentUrl = await paymentService.initPayment(bookingId, holdToken, method, ipAddr, user);
...
export const handlePaymentIpn = catchAsync(async (req: Request, res: Response) => {
  console.log('------------------------------------------------');
...
export const handlePaymentReturn = catchAsync(async (req: Request, res: Response) => {
  const { method } = req.params;
  ...
  if (payment && payment.status !== 'success') {
    ...
    if (result.code === '00') {
      ...
      await bookingService.cancelBooking(payment.bookingId.toString(), payment.userId.toString());
    }
