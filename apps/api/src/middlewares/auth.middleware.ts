...
import { IUser, IUserDocument, IUserRole } from '@shared/src/schemas';
...
export const authenticate = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (user.trang_thai !== 'active') return next(new AppError('Tài khoản bị khóa', 403));
    ...
    return res.status(401).json({
      status: 'fail',
      message: error instanceof jwt.TokenExpiredError ? 'Token đã hết hạn' : 'Xác thực lỗi',
    });
  }
});
